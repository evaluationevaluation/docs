...
### Sensus Pollucom F ("Wärmemengenzähler"; used with Hichi IR interface)
...
;- restart Tasmota with script sctivated
;- after 30s first data is retrieved and logged
;- now every 45min a new request is done, updated in WebUI (and transferred by MQTT every TPER period)
if upsecs==30 {
    ->sensor53 d1
